...
// ==============================================================================
// F3.1 - TC002: Verify Digital Submission (Project Title Missing Mismatch)
// ==============================================================================
...
TestObject errorMessage = new TestObject().addProperty("xpath", ConditionType.EQUALS, "//*[contains(@class,'error') or contains(text(),'Title') or contains(@class,'alert')]")
...
try {
    ...
    // Step 8: Leave project title blank
    WebUI.waitForElementVisible(projectTitleInput, 10, FailureHandling.STOP_ON_FAILURE)
    WebUI.clearText(projectTitleInput)
    
    // Step 9: Upload valid proposal document using direct absolute path
    ...
    // VERIFICATION: Use waitForElementVisible instead of verifyElementVisible with invalid timeout arguments
    WebUI.waitForElementVisible(errorMessage, 5, FailureHandling.STOP_ON_FAILURE)
    WebUI.verifyElementVisible(errorMessage)
    
    println("F3.1-TC002 Passed: Submission successfully intercepted due to missing project title.")

} catch (Exception e) {
    WebUI.takeScreenshot()
    println("F3.1-TC002 Failed: System incorrectly allowed submission with a blank project title.")
    throw e
} finally {
    WebUI.closeBrowser()
}