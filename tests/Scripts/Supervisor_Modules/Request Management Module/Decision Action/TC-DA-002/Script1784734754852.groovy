...
WebUI.clearText(commentBox)

WebUI.scrollToElement(rejectButton, 5)
WebUI.click(rejectButton)

String validationMessage = WebUI.executeJavaScript(
	"return document.querySelector('textarea[name=\"supervisorComment\"]').validationMessage;",
	null
)

if (validationMessage == null || validationMessage.trim() == '') {
	KeywordUtil.markFailed('Expected required-field validation message, but no validation message was returned.')
}

WebUI.verifyTextPresent('Pending', false)
WebUI.verifyElementPresent(commentBox, 10)
