...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .search-wrap svg { position: absolute; left: 10px; top: 50%; transform: translateY(-50%); color: #8ca1b6; pointer-events: none; }
        .search-wrap input { width: 200px; padding-left: 32px; }
        ...
        /*
            ...
            2: Staff ID   100px
            3: Programme  120px
            4: Classification 160px
            5: Quota Status  1fr  (takes remaining space)
            6: Actions    90px
        */
        .thead-row,
        .data-row {
            display: grid;
            grid-template-columns: 220px 100px 120px 160px 1fr 90px;
            align-items: center;
        }
        ...
        /* Each supervisor is a <form> styled as a grid row */
        .data-row {
            padding: 12px 16px;
            border-bottom: 1px solid #edf2f7;
            gap: 12px;
        ...
        .save-btn { width: 32px; height: 32px; border-radius: 7px; border: none; background: #0d5be8; color: #fff; font-weight: 900; font-size: 14px; cursor: pointer; display: grid; place-items: center; }
        .save-btn:hover { background: #0947c2; }
        .more-btn { width: 32px; height: 32px; border-radius: 7px; border: 1px solid #e2ecf5; background: #fff; color: #8a9caf; font-size: 18px; cursor: pointer; display: grid; place-items: center; line-height: 1; }
        ...
        @media (max-width: 1100px) {
            .thead-row,
            .data-row { grid-template-columns: 180px 90px 100px 150px 1fr 80px; }
        }
...
<body>
    ...
    <div class="modal-backdrop" id="accountModal" aria-hidden="true">
        <form class="account-modal" action="../../server/application/admin/updateSupervisorAccount.php" method="POST">
            ...
            <div class="modal-head">
                ...
                <button class="modal-close" type="button" data-close-account-modal>x</button>
            </div>
    ...
    <script>
        ...
        accountModal.querySelector("form").addEventListener("submit", (event) => {
            if (!confirm("Confirm supervisor account particulars update?")) {
        ...
        function openCreatePanel() {
            ...
            document.getElementById("createSupervisorID").focus();
        }
        ...
        function syncCreateQuota() {
            ...
            createQuotaID.value = String(rule.quotaID);
        }
        ...
        document.querySelectorAll("[data-close-create]").forEach((button) => {
            button.addEventListener("click", closeCreatePanel);
        });

        createClassSelect.addEventListener("change", syncCreateQuota);

        createPanel.addEventListener("reset", () => {
            window.setTimeout(syncCreateQuota, 0);
        });

        createPanel.addEventListener("submit", (event) => {
            syncCreateQuota();

            if (createQuotaID.value === "") {
                event.preventDefault();
                alert("Please select a supervisor classification.");
                return;
            }

            if (!confirm("Create this supervisor account?")) {
                event.preventDefault();
            }
        });
        ...
        document.querySelectorAll(".data-row").forEach((form) => {
            const classSelect = form.querySelector(".classification-select");
            const quotaHidden = form.querySelector('input[name="quotaID"]');

            if (!classSelect) return;

            function syncQuota() {
                const rule = quotaRules[classSelect.value];
                if (rule && quotaHidden) {
                    quotaHidden.value = String(rule.quotaID);
                }
            }

            classSelect.addEventListener("change", syncQuota);
            syncQuota();
            ...
            form.addEventListener("submit", (e) => {
                ...
                if (!rule) {
                    e.preventDefault();
                    alert("No quota rule found for the selected classification.");
                    return;
                }
                if (!confirm("Confirm supervisor classification update?")) {
                    e.preventDefault();
                }
            });
        });
